...
SPOTIFY_USER_PROFILE = {
    ...
    "images": [
        {"url": "https://i.scdn.co/image/ab6775700000ee85123456789", "height": 640, "width": 640}
    ],
...
SPOTIFY_PLAYLIST_DETAILS = {
    ...
    "images": [
        {"url": "https://i.scdn.co/image/ab67616d0000b273playlist123", "height": 640, "width": 640}
    ],
    "owner": {
        "id": "test_user_123",
        "display_name": "Test User",
    ...
    "tracks": {"href": "https://api.spotify.com/v1/playlists/test_playlist_123/tracks", "total": 3},
...
SPOTIFY_PLAYLISTS_RESPONSE = {
    ...
    "items": [
        SPOTIFY_PLAYLIST_DETAILS,
        {
            ...
            "external_urls": {"spotify": "https://open.spotify.com/playlist/test_playlist_456"},
...
SPOTIFY_ERROR_RESPONSES = {
    "unauthorized": {"error": {"status": 401, "message": "Invalid access token"}},
    "forbidden": {"error": {"status": 403, "message": "Insufficient client scope"}},
    "not_found": {"error": {"status": 404, "message": "Not found."}},
    "rate_limit": {
        "error": {"status": 429, "message": "API rate limit exceeded", "retry_after": 60}
    },
    "server_error": {"error": {"status": 500, "message": "Internal server error"}},
}


def create_spotify_user(
    user_id: str = "test_user", display_name: str = "Test User", email: str = "test@example.com"
) -> Dict[str, Any]:
...
def create_playlist_tracks_response(
    playlist_id: str, tracks: List[Dict[str, Any]], offset: int = 0, limit: int = 100
) -> Dict[str, Any]:
    ...
    return {
        ...
        "next": None
        if len(items) < limit
        else f"https://api.spotify.com/v1/playlists/{playlist_id}/tracks?offset={offset + limit}&limit={limit}",
        "offset": offset,
        "previous": None
        if offset == 0
        else f"https://api.spotify.com/v1/playlists/{playlist_id}/tracks?offset={max(0, offset - limit)}&limit={limit}",
        "total": len(items),
    }
