...
class TestUserModel:
    ...
    def test_user_playlists_relationship(self, app, db):
        ...
        playlist = Playlist(spotify_id="test_playlist", name="Test Playlist", owner_id=user.id)
...
class TestSongModel:
    ...
    def test_song_analysis_relationship(self, app, db):
        ...
        song = Song(
            spotify_id="test_song", title="Blessed Assurance", artist="Fanny Crosby", album="Hymns"
        )
        db.session.add(song)
        db.session.flush()
...
class TestAnalysisResultModel:
    ...
    def test_analysis_result_creation(self, app, db, sample_song):
        ...
        assert (
            analysis.explanation == "Song contains positive Christian themes with minor concerns."
        )
    ...
    def test_analysis_result_latest_for_song(self, app, db, sample_song):
        ...
        latest = (
            AnalysisResult.query.filter_by(song_id=sample_song.id)
...
class TestDatabaseIntegrity:
    ...
    def test_database_indexes_exist(self, app, db):
        ...
        assert spotify_id_unique or any(
            "spotify_id" in idx["column_names"] for idx in inspector.get_indexes("users")
        ...
        assert spotify_id_unique_songs or any(
            "spotify_id" in idx["column_names"] for idx in inspector.get_indexes("songs")
