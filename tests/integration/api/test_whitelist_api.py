...
from app.models.models import User, Whitelist
...
class TestWhitelistAPI:
    ...
    def test_add_whitelist_item_missing_data(self, client, authenticated_user):
        ...
        # Missing spotify_id
        response = client.post("/api/whitelist", json={"item_type": "song", "name": "Test Song"})
    ...
    def test_add_whitelist_item_invalid_type(self, client, authenticated_user):
        ...
        response = client.post(
            "/api/whitelist",
            json={"spotify_id": "test123", "item_type": "invalid_type", "name": "Test Item"},
        )
    ...
    def test_update_existing_whitelist_reason(
        self, client, authenticated_user, sample_song, sample_user, db_session
    ):
    ...
    def test_get_whitelist_items_with_data(
        self, client, authenticated_user, sample_song, sample_user, db_session
    ):
    ...
    def test_get_whitelist_items_filtered_by_type(
        self, client, authenticated_user, sample_song, sample_user, db_session
    ):
        ...
        items = [
            ...
            Whitelist(
                user_id=sample_user.id, spotify_id="artist1", item_type="artist", name="Artist 1"
            ),
        ]
    ...
    def test_remove_whitelist_item(
        self, client, authenticated_user, sample_song, sample_user, db_session
    ):
    ...
    def test_remove_whitelist_item_wrong_user(
        self, client, authenticated_user, sample_song, sample_user, db_session
    ):
    ...
    def test_clear_all_whitelist_items(
        self, client, authenticated_user, sample_song, sample_user, db_session
    ):
...
class TestWhitelistAnalysisIntegration:
    ...
    def test_whitelisted_song_analysis_optimization(
        self, client, authenticated_user, sample_song, sample_user, db_session
    ):
