...
class TestCalibrationSystem:
    ...
    def test_batch_reanalysis_performance(self):
        ...
        test_songs = [
            ...
            },
            {
                "id": 3,
                "title": "Dark Song",
                "artist": "Negative Band",
                "lyrics": "Blasphemy and hatred fill my heart",
            },
            {
                "id": 4,
                "title": "Secular Love",
                "artist": "Pop Artist",
                "lyrics": "Love makes me feel so good tonight",
            },
            {
                "id": 5,
                "title": "Mixed Content",
                "artist": "Confused Artist",
                "lyrics": "Jesus saves but money is my god",
            },
        ]
        ...
        with patch("scripts.utilities.calibration_service.CalibrationService") as mock_service:
            ...
            assert result["successful_analyses"] == len(
                test_songs
            ), "Should analyze all songs successfully"
            ...
            assert (
                result["total_time"] < expected_max_time
    ...
    def test_score_distribution_realistic(self):
        ...
        with patch("app.services.calibration_service.ScoreDistributionAnalyzer") as mock_analyzer:
            ...
            assert distribution["distribution_quality"] in [
                "good",
                "excellent",
            ], "Distribution should be well-spread"
    ...
    def test_concern_level_threshold_optimization(self):
        ...
        with patch("app.services.calibration_service.ThresholdOptimizer") as mock_optimizer:
            ...
            assert threshold_values == sorted(
                threshold_values, reverse=True
            ), "Thresholds should be in descending order"
    ...
    def test_accuracy_improvement_validation(self):
        ...
        with patch("app.services.calibration_service.PerformanceValidator") as mock_validator:
            ...
            assert (
                improvements["precision_gain"] >= 0.10
            ), "Should improve precision by at least 10%"
            assert improvements["recall_gain"] >= 0.10, "Should improve recall by at least 10%"
            assert (
                improvements["theme_detection_gain"] >= 0.15
            ), "Should improve theme detection by at least 15%"

            # Should reduce error rates
            assert (
                improvements["false_positive_reduction"] >= 0.10
            ), "Should reduce false positives by at least 10%"
            assert (
                improvements["false_negative_reduction"] >= 0.10
            ), "Should reduce false negatives by at least 10%"
            ...
            assert (
                comparison["validation_status"] == "significant_improvement"
            ), "Should validate significant improvement"
        ...
        with patch("app.services.calibration_service.SystemStabilityMonitor") as mock_monitor:
            mock_monitor.return_value.test_load_handling.return_value = {
                "batch_results": {
                    ...
                    500: {
                        "success_rate": 0.998,
                        "avg_time_per_song": 0.85,
                        "memory_usage": "312MB",
                    },
                    1000: {
                        "success_rate": 0.996,
                        "avg_time_per_song": 0.92,
                        "memory_usage": "487MB",
                    },
                    2000: {
                        "success_rate": 0.994,
                        "avg_time_per_song": 1.1,
                        "memory_usage": "723MB",
                    },
                },
            ...
            for batch_size, results in stability["batch_results"].items():
                ...
                ), f"Success rate should be 99%+ for batch size {batch_size}"
                assert (
                    results["avg_time_per_song"] <= 2.0
                ), f"Processing time should stay reasonable for batch {batch_size}"

            # Performance degradation should be acceptable
            degradation = stability["performance_degradation"]
            assert (
                degradation["time_increase_per_1000"] <= 0.5
            ), "Time increase should be manageable"
            assert degradation["success_rate_stable"] is True, "Success rate should remain stable"

            # System should be assessed as stable
            assert (
                stability["stability_assessment"] == "stable_under_load"
            ), "System should handle load well"
            assert (
                stability["recommended_batch_size"] >= 500
    ...
    def test_calibration_config_management(self):
        ...
        with patch("app.services.calibration_service.CalibrationConfigManager") as mock_config:
            ...
            assert (
                len(loaded_config["score_thresholds"]) >= 4
            ), "Should have multiple concern levels"
