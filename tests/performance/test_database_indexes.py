...
class TestDatabaseIndexPerformance:
    ...
    def test_progress_api_performance_benchmark(self, client, app):
        ...
        with app.app_context():
            ...
            ), f"Progress API took {response_time_ms:.1f}ms, target was 400ms"
    ...
    def test_performance_api_benchmark(self, client, app):
        ...
        with app.app_context():
            ...
            ), f"Performance API took {response_time_ms:.1f}ms, target was 500ms"
    ...
    def test_playlist_query_performance(self, client, app):
        ...
        with app.app_context():
            ...
            ), f"Large playlist query took {response_time_ms:.1f}ms, target was 200ms"
    ...
    def test_dashboard_performance_with_many_playlists(self, client, app):
        ...
        with app.app_context():
            ...
            assert response.status_code == 200
            # Relax under test environment to avoid flakiness
            target = 150 if app.config.get("TESTING") else 100
            assert (
                response_time_ms < target
            ), f"Dashboard took {response_time_ms:.1f}ms, target was {target}ms"
    ...
    def test_analysis_results_query_performance(self, app):
        ...
        with app.app_context():
            ...
            assert (
                query_time_ms < 50
            ), f"Analysis results query took {query_time_ms:.1f}ms, target was 50ms"
    ...
    def test_playlist_songs_join_performance(self, app):
        ...
        with app.app_context():
            ...
            assert (
                query_time_ms < 30
            ), f"Playlist songs join took {query_time_ms:.1f}ms, target was 30ms"
    ...
    def test_user_playlists_query_performance(self, app):
        ...
        with app.app_context():
            ...
            assert (
                query_time_ms < 20
            ), f"User playlists query took {query_time_ms:.1f}ms, target was 20ms"
    ...
    def _create_large_playlist(self, song_count=100):
        ...
        for i, song in enumerate(existing_songs[:song_count]):
            playlist_song = PlaylistSong(
                playlist_id=playlist.id, song_id=song.id, track_position=i + 1
            )
...
class TestDatabaseConsistency:
    ...
    def _verify_foreign_key_constraints(self):
        ...
        orphaned_playlists = (
            ...
            .filter(~Playlist.owner_id.in_(db.session.query(User.id)))
            .count()
        )
        assert orphaned_playlists == 0, f"Found {orphaned_playlists} orphaned playlists"

        # Check playlist-song relationships
        orphaned_playlist_songs = (
            db.session.query(PlaylistSong)
            .filter(~PlaylistSong.playlist_id.in_(db.session.query(Playlist.id)))
            .count()
        )
        assert (
            orphaned_playlist_songs == 0
        ), f"Found {orphaned_playlist_songs} orphaned playlist songs"

        # Check analysis-song relationships
        orphaned_analyses = (
            db.session.query(AnalysisResult)
            .filter(~AnalysisResult.song_id.in_(db.session.query(Song.id)))
            .count()
        )
    ...
    def _verify_data_consistency(self):
        ...
        playlists_with_songs = (
            ...
            .group_by(Playlist.id, Playlist.track_count)
            .all()
        )
        ...
        for playlist_id, declared_count, actual_count in playlists_with_songs:
            if declared_count is not None and actual_count != declared_count:
                print(
                    f"Warning: Playlist {playlist_id} declares {declared_count} tracks but has {actual_count}"
                )
...
class TestPerformanceRegression:
    ...
    def test_all_existing_endpoints_maintain_performance(self, client, app, performance_baseline):
        ...
        with app.app_context():
            for endpoint, method in endpoints_to_test:
                ...
                ), f"{endpoint} took {response_time_ms:.1f}ms (too slow)"
    ...
    def test_memory_usage_acceptable(self, app):
        ...
        print(
            f"Memory usage: {memory_before:.1f}MB -> {memory_after:.1f}MB (+{memory_increase:.1f}MB)"
        )
