...
class TestDatabaseQueryPerformance:
    ...
    def test_song_count_query_performance(self, app):
        ...
        with app.app_context():
            ...
            assert count >= 100
            assert query_time_ms < 50, f"Song count query took {query_time_ms:.1f}ms, target was 50ms"
    ...
    def test_analysis_results_with_songs_query(self, app):
        ...
        with app.app_context():
            ...
            assert query_time_ms < 100, f"Analysis results join query took {query_time_ms:.1f}ms, target was 100ms"
            print(f"Analysis results join query: {query_time_ms:.1f}ms ({len(recent_results)} results)")
    ...
    def test_playlist_songs_join_query(self, app):
        ...
        with app.app_context():
            ...
            assert len(songs) <= 25
            assert query_time_ms < 50, f"Playlist songs join took {query_time_ms:.1f}ms, target was 50ms"
    ...
    def test_user_playlists_query(self, app):
        ...
        with app.app_context():
            ...
            assert query_time_ms < 30, f"User playlists query took {query_time_ms:.1f}ms, target was 30ms"
    ...
    def test_analysis_status_aggregation(self, app):
        ...
        with app.app_context():
            ...
            completed_analyses = db.session.query(AnalysisResult)\
            ...
            assert query_time_ms < 100, f"Analysis aggregation took {query_time_ms:.1f}ms, target was 100ms"
            print(f"Analysis aggregation: {query_time_ms:.1f}ms (Total: {total_songs}, Completed: {completed_analyses})")
...
class TestDatabaseIndexAnalysis:
    ...
    def test_identify_slow_queries(self, app):
        ...
        with app.app_context():
            ...
            start_time = time.time()
            db.session.query(AnalysisResult).order_by(AnalysisResult.analyzed_at.desc()).limit(10).all()
            query_time = (time.time() - start_time) * 1000
            if query_time > 20:
            ...
            if slow_queries:
                ...
                print("  ✅ All test queries performed well (may need more data to see bottlenecks)")
