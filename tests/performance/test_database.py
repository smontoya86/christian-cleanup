...
from app.models.models import db, User, Playlist, Song, AnalysisResult, PlaylistSong
    ...
    def test_playlist_songs_with_analysis_query_performance(self):
        ...
        with self.app.app_context():
            ...
            def query_playlist_songs_with_analysis():
                ...
                songs_with_analysis = db.session.query(
                    Song, AnalysisResult
                ).outerjoin(
    ...
    def test_analysis_aggregation_query_performance(self):
        ...
        with self.app.app_context():
            ...
            def query_analysis_aggregations():
                ...
                concern_stats = db.session.query(
                ...
                user_stats = db.session.query(
                    ...
                    Playlist, User.id == Playlist.owner_id
                ).join(
                    ...
                    AnalysisResult, Song.id == AnalysisResult.song_id
                ).group_by(User.id, User.display_name).all()
                
                return {
                    'concern_stats': [(level, count, float(avg)) for level, count, avg in concern_stats],
                    'overall_stats': {
                    ...
                    'user_stats': [(uid, name, count, float(avg)) for uid, name, count, avg in user_stats]
                }
        ...
        with self.app.app_context():
            ...
            def complex_join_query():
                ...
                results = db.session.query(
                    ...
                    func.count(
                        db.case(
                            (AnalysisResult.concern_level == 'high', 1),
                            else_=None
                        )
                    ...
                    User.id, User.display_name, Playlist.id, Playlist.name
                ).having(
                    func.count(Song.id) > 0
                ).order_by(
                    func.avg(AnalysisResult.score).desc().nullslast()
                ).all()
            ...
            def paginated_query():
                ...
                for page in range(1, 6):  # Test first 5 pages
                    ...
                    songs = db.session.query(Song).join(
                        AnalysisResult, Song.id == AnalysisResult.song_id, isouter=True
                    ).order_by(
                        Song.title
