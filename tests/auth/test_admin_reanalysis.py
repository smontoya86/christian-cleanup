...
class TestAdminReanalysis:
    ...
    def test_admin_reanalysis_endpoint_allows_admin(
        self, client, app, admin_user, test_user, db_session
    ):
        """Test that admin can trigger re-analysis for a user"""
        # Create some test data
        playlist = Playlist(
            spotify_id="test_playlist_123", name="Test Playlist", owner_id=test_user.id
        )
    ...
    def test_admin_reanalysis_resets_existing_analysis(
        self, client, app, admin_user, test_user, db_session
    ):
        """Test that re-analysis properly resets existing analysis results"""
        # Create test data with completed analysis
        playlist = Playlist(
            spotify_id="test_playlist_reset", name="Test Playlist Reset", owner_id=test_user.id
        )
        db_session.add(playlist)

        song = Song(
            spotify_id="test_song_reset", title="Test Song Reset", artist="Test Artist Reset"
        )
        db_session.add(song)
        db_session.commit()
