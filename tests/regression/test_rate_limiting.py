    ...
    def test_concurrent_lyrics_requests_handle_limits(self, app, lyrics_fetcher):
        ...
        with app.app_context():
                    ...
                    with app.app_context():
                        ...
                        with patch.object(lyrics_fetcher, "fetch_lyrics", return_value=None):
                            result = lyrics_fetcher.fetch_lyrics(
                                "Test Artist", f"Test Song {song_id}"
                            )
            ...
            critical_errors = [
                e
                for e in errors
                if any(err_type in e.lower() for err_type in ["crash", "fatal", "unhandled"])
            ]

            assert (
                len(critical_errors) == 0
            ), f"Critical errors in concurrent requests: {critical_errors}"
