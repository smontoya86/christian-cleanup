...
class TestPhase1Regression:
    ...
    def test_database_models_unchanged(self, app):
        ...
        with app.app_context():
            ...
            new_analysis = AnalysisResult(song_id=new_song.id, status="pending", score=None)
    ...
    def test_api_endpoints_functional(self, app):
        ...
        with app.test_client() as client:
            with app.app_context():
                ...
                # Normalize converter types for robust comparison
                def _normalize(rule_path: str) -> str:
                    return rule_path.replace("<int:", "<")

                normalized_rules = [_normalize(r) for r in rules]

                for route in expected_routes:
                    normalized_expected = _normalize(route)
                    assert (
                        normalized_expected in normalized_rules
                    ), f"Route pattern {route} should still be registered"
    ...
    def test_unified_analysis_service_compatibility(self, app):
        ...
        with app.app_context():
            ...
            assert hasattr(
                service, "analyze_song_complete"
            ), "Should have analyze_song_complete method"

            # Test that the analysis service is the expected type
            assert isinstance(
                service.analysis_service, SimplifiedChristianAnalysisService
    ...
    def test_existing_test_compatibility(self, app):
        ...
        with app.app_context():
            ...
            analysis = AnalysisResult(
                song_id=song.id, status="completed", score=80.0, concern_level="low"
            )
