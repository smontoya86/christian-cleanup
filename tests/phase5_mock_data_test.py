...
class TestPhase5MockData:
    ...
    def test_application_is_running(self):
        if not os.getenv("RUN_E2E_LIVE"):
            pytest.skip("Skipping live server test; set RUN_E2E_LIVE=1 to enable")
        ...
        for song_id in range(1, 8):  # Test songs created by mock script
            ...
            if response.status_code == 200:
                ...
                assert any(
                    ...
                    for keyword in ["analysis", "score", "christian", "concern", "lyrics"]
                )
                break
    ...
    def test_mock_data_variety(self):
        ...
        if dashboard_response.status_code == 200:
            ...
            found_playlists = sum(
                1 for playlist in expected_playlists if playlist in dashboard_text
            )
