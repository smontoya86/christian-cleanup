...
class TestDatabaseIndexes:
    ...
    def test_required_indexes_exist(self):
        ...
        assert any('spotify_id' in idx['column_names'] for idx in songs_indexes), \
            "songs.spotify_id index should exist"
        
        # Check playlists table indexes
        playlists_indexes = inspector.get_indexes('playlists')
        assert any('spotify_id' in idx['column_names'] for idx in playlists_indexes), \
            "playlists.spotify_id index should exist"
        assert any('owner_id' in idx['column_names'] for idx in playlists_indexes), \
        ...
        assert any('song_id' in idx['column_names'] for idx in analysis_indexes), \
            "analysis_results.song_id index should exist"
        assert any('status' in idx['column_names'] for idx in analysis_indexes), \
        ...
        assert any('playlist_id' in idx['column_names'] for idx in playlist_songs_indexes), \
            "playlist_songs.playlist_id index should exist"
        assert any('song_id' in idx['column_names'] for idx in playlist_songs_indexes), \
    ...
    def test_foreign_key_join_performance(self):
        ...
        start_time = time.time()
        result = db.session.query(Song, AnalysisResult).join(AnalysisResult).filter(
            Song.spotify_id == 'test_song_123'
        ).first()
        join_time = (time.time() - start_time) * 1000  # Convert to ms
        
        assert result is not None
        assert join_time < 50, f"Join query took {join_time:.1f}ms, should be < 50ms"
...
class TestQueryOptimization:
    ...
    def test_playlist_songs_eager_loading(self):
        ...
        start_time = time.time()
        playlist_with_songs = Playlist.query.options(
...
class TestBatchOperations:
    ...
    def test_bulk_analysis_updates(self):
        ...
        assert bulk_update_time < 200, f"Bulk update took {bulk_update_time:.1f}ms, should be < 200ms" 