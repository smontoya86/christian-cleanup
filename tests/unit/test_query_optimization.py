...
class TestQueryOptimization:
    ...
    def test_optimized_playlist_loading_with_eager_loading(self):
        ...
        optimized_playlists = Playlist.query.filter_by(owner_id=user.id).options(
    ...
    def test_query_optimization_performance_improvement(self):
        ...
        assert improvement_ratio >= 2, f"Expected at least 2x query reduction, got {improvement_ratio:.1f}x"
    ...
    def test_dashboard_query_optimization(self):
        ...
        assert dashboard_query_count <= 4, f"Expected ≤4 optimized dashboard queries, got {dashboard_query_count}"
    ...
    def test_playlist_detail_optimization(self):
        ...
        playlist_with_songs = Playlist.query.filter_by(id=playlist.id).options(
            subqueryload(Playlist.song_associations).joinedload(PlaylistSong.song)
        ).first()
        ...
        assert detail_query_count <= 6, f"Expected ≤6 queries for playlist detail, got {detail_query_count}"
