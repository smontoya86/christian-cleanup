...
class TestSimplifiedChristianAnalysisService:
    ...
    def test_christian_content_analysis_quality(self, service, sample_christian_song):
        ...
        with patch.object(service, 'ai_analyzer') as mock_ai:
            ...
            assert result.scoring_results['final_score'] >= 70, "Strong Christian content should score 70+"
            assert len(result.biblical_analysis['themes']) >= 3, "Should identify multiple Christian themes"
            assert result.scoring_results['quality_level'] in ['Very Low', 'Low'], "Should have minimal concerns"
            assert 'supporting_scripture' in result.biblical_analysis, "Should provide scripture references"
    ...
    def test_concerning_content_proper_flagging(self, service, sample_concerning_song):
        ...
        with patch.object(service, 'ai_analyzer') as mock_ai:
            ...
            assert result.scoring_results['final_score'] <= 40, "Concerning content should score ≤40"
            assert len(result.content_analysis['concern_flags']) >= 1, "Should flag concerning elements"
            assert result.scoring_results['quality_level'] == 'High', "Should have high concern level"
    ...
    def test_nuanced_content_handling(self, service, sample_nuanced_song):
        ...
        with patch.object(service, 'ai_analyzer') as mock_ai:
            ...
            assert 40 <= result.scoring_results['final_score'] <= 80, "Nuanced content should score 40-80"
            assert result.scoring_results['quality_level'] in ['Low', 'Medium'], "Should have moderate concern"
    ...
    def test_educational_value_requirements(self, service, sample_christian_song):
        ...
        with patch.object(service, 'ai_analyzer') as mock_ai, \
            ...
            christian_concepts = ['grace', 'salvation', 'biblical', 'faith', 'christian', 'scripture']
            assert any(concept in combined_text for concept in christian_concepts), \
    ...
    def test_error_handling_robustness(self, service):
        ...
        with patch.object(service, 'ai_analyzer') as mock_ai:
            ...
            mock_ai.analyze_comprehensive.side_effect = Exception("AI service temporarily unavailable")
    ...
    def test_scripture_mapping_integration(self, service):
        ...
        with patch.object(service, 'ai_analyzer') as mock_ai, \
            ...
            assert 'love' in call_args[0] or 'forgiveness' in call_args[0], "Should pass detected themes"
