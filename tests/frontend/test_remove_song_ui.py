...
from app import db
from app.models.models import Playlist, Song, PlaylistSong, AnalysisResult, User
...
class TestRemoveSongPlaylistDetailUI:
    ...
    def test_remove_song_option_appears_in_dropdown_for_all_songs(self, client, authenticated_client, sample_user):
        ...
        assert len(remove_forms) >= 3, f"Expected remove song forms for all 3 songs, found {len(remove_forms)}"
    ...
    def test_remove_song_button_has_correct_styling_and_confirmation(self, client, authenticated_client, sample_user):
        ...
        remove_button = soup.find('button', lambda tag: tag and 'Remove from Playlist' in tag.get_text())
        ...
        assert 'text-danger' in remove_button.get('class', []), "Remove button should have danger styling"
...
class TestRemoveSongDetailUI:
    ...
    def test_remove_song_button_appears_when_viewed_from_playlist(self, client, authenticated_client, sample_user):
    ...
    def test_remove_song_button_not_shown_when_no_playlist_context(self, client, authenticated_client, sample_user):
        ...
        if playlist_name_in_html:
            ...
            assert remove_button is not None, "Remove button should be present when playlist is auto-detected"
        else:
            # If no playlist context, button should not be there
            assert remove_button is None, "Remove from Playlist button should NOT be present without playlist context"

    def test_remove_song_button_placement_in_actions_section(self, client, authenticated_client, sample_user):
        ...
        remove_button = actions_card.find('button', lambda tag: tag and 'Remove from Playlist' in tag.get_text())
        assert remove_button is not None, "Remove button should be in actions section"

    def test_remove_song_form_includes_correct_playlist_context(self, client, authenticated_client, sample_user):
        ...
        assert f'/remove_song/{playlist.id}/{song.id}' in action, f"Form action should include correct IDs, found: {action}"


class TestRemoveSongFunctionalFlow:
    """Test complete functional flow of remove song feature"""

    def test_remove_song_preserves_other_songs_in_playlist(self, client, authenticated_client, sample_user):
    ...
    def test_remove_song_ui_provides_appropriate_feedback_elements(self, client, authenticated_client, sample_user):
        ...
        if remove_button:
            ...
            assert ('fa-' in button_html or 'fas ' in button_html), "Remove button should include an icon for visual clarity" 