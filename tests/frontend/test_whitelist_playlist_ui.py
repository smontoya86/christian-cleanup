...
from app import db
from app.models.models import Song, Playlist, PlaylistSong, Whitelist, AnalysisResult, User
...
class TestWhitelistPlaylistUI:
    ...
    def test_playlist_detail_overflow_menu_analyzed_songs(self, authenticated_client, sample_playlist_with_songs):
        """Test that analyzed songs show overflow menu with re-analyze option"""
        playlist_id = sample_playlist_with_songs['playlist'].id
        
        response = authenticated_client.get(url_for('main.playlist_detail', playlist_id=playlist_id))
        ...
        for row in analyzed_rows:
            ...
            if score_cell and 'Not Analyzed' not in score_cell.get_text():
                ...
                item_texts = [item.get_text().strip() for item in dropdown_items if item.get_text().strip()]
                
                # Should have View Details and Re-analyze options
                assert any('View Details' in text for text in item_texts), "Should have View Details option"
                assert any('Re-analyze' in text for text in item_texts), "Should have Re-analyze option"
    ...
    def test_primary_buttons_no_icons(self, authenticated_client, sample_playlist_with_songs):
        ...
        primary_buttons = soup.find_all('button', class_=['btn-success', 'btn-warning', 'btn-primary'])
        ...
        for button in all_primary_actions:
            if 'dropdown-toggle' not in button.get('class', []):
                ...
                if button_text in ['Whitelist', 'Review', 'Analyze']:
                    ...
                    assert len(text_icons) == 0, f"Primary button '{button_text}' should not have icons"
    
    def test_overflow_menu_whitelist_anyway_high_concern(self, authenticated_client, sample_playlist_with_songs):
    ...
    def test_whitelisted_songs_overflow_menu(self, authenticated_client, sample_playlist_with_songs):
        ...
        for badge in whitelisted_badges:
            if 'Whitelisted' in badge.get_text():
                ...
                if row:
                    ...
                    dropdown_items = dropdown_menu.find_all('li')
                    item_texts = [item.get_text().strip() for item in dropdown_items if item.get_text().strip()]
                    assert any('View Details' in text for text in item_texts), "Should have View Details"
                    assert any('Remove from Whitelist' in text for text in item_texts), "Should have Remove option"
    
    def test_actions_column_responsive_width(self, authenticated_client, sample_playlist_with_songs):
...
class TestPlaylistWhitelistUI:
    ...
    def test_whitelist_button_appears_for_analyzed_songs(self, client, authenticated_client, sample_song, sample_user):
        ...
        whitelist_buttons = soup.find_all('button', string=lambda text: text and 'Whitelist' in text)
        if not whitelist_buttons:
            # Also check button content for mixed icon+text buttons
            whitelist_buttons = soup.find_all('button', title=lambda title: title and 'Whitelist' in title)
        assert len(whitelist_buttons) >= 1
        
        # Should have form with correct action
        whitelist_forms = soup.find_all('form', action=lambda action: action and 'whitelist_song' in action)
        assert len(whitelist_forms) >= 1

    def test_whitelist_action_from_playlist_works(self, client, authenticated_client, sample_song, sample_user):
        ...
        response = authenticated_client.post(f'/whitelist_song/{sample_song.id}', 
    ...
    def test_whitelisted_song_shows_remove_option(self, client, authenticated_client, sample_song, sample_user):
        ...
        assert check_whitelist is not None, f"Whitelist entry not found for song {sample_song.spotify_id}"
        ...
        assert len(whitelisted_badges) >= 1, f"Expected whitelisted badge but found badges: {[b.get_text() for b in all_badges]}"

    def test_remove_whitelist_from_playlist_works(self, client, authenticated_client, sample_song, sample_user):
        ...
        response = authenticated_client.post(f'/remove_whitelist/{sample_song.id}',
                                           follow_redirects=False)
