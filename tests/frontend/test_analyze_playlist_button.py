...
class TestAnalyzePlaylistButton:
    ...
    def test_button_shows_analyze_for_unanalyzed_playlist(
        ...
        playlist = Playlist(
            name="Unanalyzed Playlist", spotify_id="unanalyzed_123", owner_id=sample_user.id
        )
        ...
        response = authenticated_client.get(
            url_for("main.playlist_detail", playlist_id=playlist.id)
        )
        assert response.status_code == 200
    ...
    def test_button_shows_reanalyze_for_analyzed_playlist(
        self, authenticated_client, sample_user, db_session
    ):
        """Test that button shows 'Re-analyze All Songs' for playlist with all songs analyzed"""
        # Create playlist with analyzed songs
        playlist = Playlist(
            name="Analyzed Playlist", spotify_id="analyzed_123", owner_id=sample_user.id
        )
        db_session.add(playlist)
        db_session.flush()
    ...
    def test_button_shows_analyze_for_partially_analyzed_playlist(
        self, authenticated_client, sample_user, db_session
    ):
        ...
        playlist = Playlist(
            name="Partially Analyzed Playlist", spotify_id="partial_123", owner_id=sample_user.id
        )
        db_session.add(playlist)
        db_session.flush()
    ...
    def test_analyze_playlist_route_works(
        self, mock_service, authenticated_client, sample_playlist_with_songs
    ):
        ...
        response = authenticated_client.post(
            url_for("main.analyze_playlist", playlist_id=playlist.id)
        )
    ...
    def test_analyze_button_has_data_attributes(self, authenticated_client, sample_playlist):
        ...
        assert playlist_id == str(
            sample_playlist.id
        ), f"Expected playlist ID {sample_playlist.id}, got {playlist_id}"
        assert (
            playlist_name == sample_playlist.name
        ), f"Expected playlist name '{sample_playlist.name}', got '{playlist_name}'"

    def test_analyze_playlist_button_integration(
    ...
    def test_playlist_analysis_progress_tracking(
        self, authenticated_client, sample_playlist_with_songs
    ):
    ...
    def test_playlist_analysis_ajax_response(
        self, authenticated_client, sample_playlist_with_songs
    ):
    ...
    def test_playlist_analysis_non_ajax_response(
        self, authenticated_client, sample_playlist_with_songs
    ):
        ...
        assert (
            f"/playlist/{playlist.id}" in response.location
        ), "Should redirect to playlist detail page"

    def test_playlist_analysis_complete_workflow(
        self, authenticated_client, sample_playlist_with_songs
    ):
        ...
        progress_response = authenticated_client.get(
            f"/api/playlists/{playlist.id}/analysis-status"
        )
