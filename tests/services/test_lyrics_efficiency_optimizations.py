...
class TestLyricsEfficiencyOptimizations:
    ...
    def test_request_deduplication_skips_analyzed_songs(self, app, db_session):
        ...
        assert result['total_filtered'] == 2, f"Expected 2 filtered songs, got {result['total_filtered']}"
        assert result['needs_analysis'] == 1, f"Expected 1 song needing analysis, got {result['needs_analysis']}"
    ...
    def test_request_deduplication_includes_failed_when_retry_enabled(self, app, db_session):
        ...
        assert result2['song_ids'] == [song.id], "Failed song should be included when retry_failed=True"
    ...
    def test_request_deduplication_performance_with_large_batch(self, app, db_session):
        ...
        songs = []
        for i in range(100):
            song = Song(spotify_id=f"batch_track_{i}", title=f"Batch Song {i}", artist=f"Batch Artist {i}")
            songs.append(song)
    ...
    def test_batch_database_operations_reduce_commit_overhead(self, app, db_session):
        ...
        songs = []
        for i in range(10):
            song = Song(spotify_id=f"batch_db_track_{i}", title=f"Batch Song {i}", artist=f"Batch Artist {i}", lyrics="Test lyrics")
            songs.append(song)
        ...
        with patch.object(service, 'analyze_song_complete') as mock_analyze:
            ...
            assert commit_count <= 3, f"Expected ≤3 commits for batched processing, got {commit_count}"
    ...
    def test_batch_lyrics_cache_operations_are_efficient(self, app, db_session):
        ...
        for i in range(20):
            cache_operations.append({
                ...
                'title': f'Test Song {i}',
                'lyrics': f'Test lyrics {i}' if i % 2 == 0 else None,  # Mix of success and failure
                'source': 'TestProvider'
            })
        ...
        print(f"Cache operations resulted in {commit_count} commits for {len(cache_operations)} operations")
    ...
    def test_database_bulk_insert_vs_individual_inserts(self, app, db_session):
        ...
        individual_entries = []
        for i in range(50):
            entry = LyricsCache.cache_lyrics(f'Individual Artist {i}', f'Individual Song {i}', f'Lyrics {i}', 'TestProvider')
        ...
        assert bulk_duration < individual_duration / 2, "Bulk inserts should be significantly faster than individual inserts"
