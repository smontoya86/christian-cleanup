...
def assert_has_attributes(
    obj: Any, attributes: List[str], message: str = "Missing expected attributes"
) -> None:
    """
...
def assert_analysis_result_valid(result: Any, expected_safe: Optional[bool] = None) -> None:
    ...
    for flag in expected_flags:
        ...
        assert isinstance(
            result.content_flags[flag], bool
        ), f"Content flag {flag} should be boolean"
    ...
    if expected_safe is not None:
        ...
        ), f"Expected safety: {expected_safe}, got: {result.is_safe}"
...
def create_test_playlist_data(
    playlist_id: str = "test_playlist", name: str = "Test Playlist", track_count: int = 0
) -> Dict[str, Any]:
    """
...
def assert_flash_message(
    response, expected_category: str, expected_message: Optional[str] = None
) -> None:
    """
