...
from ..mocks import MockServiceFactory, MockSpotifyService
from ..mocks.analysis_mocks import (
    MockUnifiedAnalysisService,
    create_mock_analysis_result,
    create_mock_song_analyzer,
)


@contextmanager
def setup_spotify_mocks(
    test_data: Optional[Dict[str, Any]] = None,
) -> Generator[MockSpotifyService, None, None]:
    """
...
def setup_analysis_mocks(
    ...
    with (
        patch("app.utils.analysis.legacy_adapter.SongAnalyzer", return_value=mock_song_analyzer),
        patch(
            "app.services.unified_analysis_service.UnifiedAnalysisService",
...
@contextmanager
def setup_all_mocks(
    spotify_data: Optional[Dict[str, Any]] = None, analysis_data: Optional[Dict[str, Any]] = None
) -> Generator[Dict[str, Any], None, None]:
    """
    ...
    """
    with (
        setup_spotify_mocks(spotify_data) as spotify_mock,
        setup_analysis_mocks(analysis_data) as analysis_mocks,
    ):
        yield {"spotify": spotify_mock, **analysis_mocks}


def create_mock_user(
    user_id: int = 1, spotify_id: str = "test_user_123", access_token: str = "mock_access_token"
) -> Mock:
    """
