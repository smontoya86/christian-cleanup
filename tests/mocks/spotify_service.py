...
class MockSpotifyService(BaseMockService):
    ...
    def _maybe_raise_error(self, method_name: str) -> None:
        ...
        if self._should_fail:
            error_response = self.test_data["error_responses"].get(
                self._error_type, self.test_data["error_responses"]["server_error"]
            )
    ...
    def get_user_playlists(self, limit: int = 20, offset: int = 0) -> Optional[Dict[str, Any]]:
        ...
        if offset + limit < len(all_playlists):
            response["next"] = (
                f"https://api.spotify.com/v1/me/playlists?offset={offset + limit}&limit={limit}"
            )
        else:
            response["next"] = None
        ...
        if offset > 0:
            response["previous"] = (
                f"https://api.spotify.com/v1/me/playlists?offset={max(0, offset - limit)}&limit={limit}"
            )
        else:
    ...
    def get_playlist_items(
        self, playlist_id: str, fields: Optional[str] = None, limit: int = 100, offset: int = 0
        ...
        self._simulate_network_call(
            "get_playlist_items", playlist_id=playlist_id, fields=fields, limit=limit, offset=offset
        )
        ...
        if offset + limit < len(all_items):
            response["next"] = (
                f"https://api.spotify.com/v1/playlists/{playlist_id}/tracks?offset={offset + limit}&limit={limit}"
            )
        else:
            response["next"] = None
        ...
        if offset > 0:
            response["previous"] = (
                f"https://api.spotify.com/v1/playlists/{playlist_id}/tracks?offset={max(0, offset - limit)}&limit={limit}"
            )
        else:
            response["previous"] = None
    ...
    def get_playlist_details(self, user, spotify_playlist_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        self._simulate_network_call(
            "get_playlist_details", user=user, spotify_playlist_id=spotify_playlist_id
        )
    ...
    def get_multiple_track_details(
        self, user, track_uris: List[str]
        ...
        """
        self._simulate_network_call("get_multiple_track_details", user=user, track_uris=track_uris)
    ...
    def add_tracks_to_spotify_playlist(
        self, user, spotify_playlist_id: str, track_uris: List[str]
    ) -> bool:
        """
    ...
    def reorder_tracks_in_spotify_playlist(
        self,
        user,
        ...
        """
        self._simulate_network_call(
            "reorder_tracks_in_spotify_playlist",
            user=user,
            spotify_playlist_id=spotify_playlist_id,
    ...
    def get_spotify_data(
        self, endpoint: str, access_token: str, params: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        self._simulate_network_call(
            "get_spotify_data", endpoint=endpoint, access_token=access_token, params=params
        )
    ...
    def add_test_tracks_to_playlist(self, playlist_id: str, tracks: List[Dict[str, Any]]) -> None:
        ...
        self._playlist_tracks[playlist_id]["total"] = len(
            self._playlist_tracks[playlist_id]["items"]
        )
