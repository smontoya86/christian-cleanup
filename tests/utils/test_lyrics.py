...
class TestLyricsFetcher:
    ...
    def test_fetch_lyrics_success_multi_provider(self, mock_sleep):
        ...
        with patch.object(
            self.fetcher.providers[0], "fetch_lyrics", return_value=None
        ) as mock_lrclib:
            with patch.object(
                self.fetcher.providers[1], "fetch_lyrics", return_value=None
            ) as mock_lyricsovh:
                with patch.object(
                    self.fetcher.providers[2],
                    "fetch_lyrics",
    ...
    def test_fetch_lyrics_first_provider_success(self, mock_sleep):
        ...
        with patch.object(
            self.fetcher.providers[0], "fetch_lyrics", return_value="Lyrics from LRCLib"
        ) as mock_lrclib:
            with patch.object(
                self.fetcher.providers[1], "fetch_lyrics", return_value=None
            ) as mock_lyricsovh:
                with patch.object(
                    self.fetcher.providers[2], "fetch_lyrics", return_value=None
                ) as mock_genius:
    ...
    def test_fetch_lyrics_with_caching(self, mock_sleep):
        ...
        with patch.object(
            self.fetcher.providers[0], "fetch_lyrics", return_value="Test lyrics content"
        ) as mock_lrclib:
    ...
    def test_fetch_lyrics_exception_handling(self, mock_sleep):
        ...
        with patch.object(
            self.fetcher.providers[0], "fetch_lyrics", side_effect=Exception("API Error")
        ) as mock_lrclib:
            with patch.object(
                self.fetcher.providers[1], "fetch_lyrics", return_value="Fallback lyrics"
            ) as mock_lyricsovh:
    ...
    def test_get_synced_lyrics(self):
        ...
        with patch.object(
            self.fetcher.providers[0], "fetch_lyrics", return_value="[00:12.34]Synced lyrics"
        ) as mock_lrclib:
    ...
    def test_multi_provider_fallback_chain(self):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    self.fetcher.providers[2], "fetch_lyrics", return_value="Genius lyrics"
                ) as mock_genius:
                    result = self.fetcher.fetch_lyrics("Test Song", "Test Artist")
