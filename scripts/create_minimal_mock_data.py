...
def create_mock_data():
    ...
    with app.app_context():
        ...
        try:
            ...
            user1_exists = conn.execute(text("SELECT id FROM users WHERE spotify_id = 'test_user_1'")).fetchone()
            user2_exists = conn.execute(text("SELECT id FROM users WHERE spotify_id = 'test_user_2'")).fetchone()
            ...
            # Get user IDs
            user1_id = conn.execute(text("SELECT id FROM users WHERE spotify_id = 'test_user_1'")).fetchone()[0]
            user2_id = conn.execute(text("SELECT id FROM users WHERE spotify_id = 'test_user_2'")).fetchone()[0]
            
            # Create test songs
            print("\n🎵 Creating mock songs...")
            
            songs_data = [
                ('christian_song_1', 'Amazing Grace', 'Chris Tomlin', 'Worship Collection', 240000, 
                 'Amazing grace, how sweet the sound\nThat saved a wretch like me', False),
                ('christian_song_2', 'How Great Thou Art', 'Hillsong Worship', 'Modern Hymns', 280000,
                 'O Lord my God, when I in awesome wonder\nConsider all the worlds thy hands have made', False),
                ('questionable_song_1', 'Party All Night', 'Secular Artist', 'Club Hits', 200000,
                 'We\'re gonna party all night long\nDrinking till the break of dawn', False),
                ('clean_song_1', 'Beautiful Day', 'Positive Vibes', 'Sunshine Collection', 190000,
                 'It\'s a beautiful day, the sun is shining bright\nEverything feels right', False)
            ]
            
            song_ids = []
            for spotify_id, title, artist, album, duration, lyrics, explicit in songs_data:
                # Check if song exists
                existing_song = conn.execute(text("SELECT id FROM songs WHERE spotify_id = :spotify_id"), {'spotify_id': spotify_id}).fetchone()
            ...
            analysis_data = [
                ...
                (song_ids[2], 'completed', 45.0, 'High', '[]', '{"substance_abuse": ["drinking"], "materialism": ["money", "cars"]}'),
            ...
            for song_id, status, score, concern_level, themes, problematic_content in analysis_data:
                ...
                existing_analysis = conn.execute(text("SELECT id FROM analysis_results WHERE song_id = :song_id"), {'song_id': song_id}).fetchone()
            ...
            playlists_data = [
                (user1_id, 'playlist_worship_1', 'Sunday Worship', 'Songs for Sunday morning worship service', 'https://via.placeholder.com/300x300?text=Worship', 2, [0, 1]),
                (user1_id, 'playlist_mixed_1', 'Mixed Playlist', 'A mix of different music styles', 'https://via.placeholder.com/300x300?text=Mixed', 3, [0, 2, 3]),
                (user2_id, 'playlist_christian_1', 'Christian Favorites', 'My favorite Christian songs', 'https://via.placeholder.com/300x300?text=Christian', 3, [0, 1, 3]),
                (user2_id, 'playlist_review_1', 'Needs Review', 'Playlist that needs content review', 'https://via.placeholder.com/300x300?text=Review', 1, [2])
            ]
            
            for owner_id, spotify_id, name, description, image_url, track_count, song_indices in playlists_data:
                # Check if playlist exists
                existing_playlist = conn.execute(text("SELECT id FROM playlists WHERE spotify_id = :spotify_id AND owner_id = :owner_id"), 
                                                {'spotify_id': spotify_id, 'owner_id': owner_id}).fetchone()
                ...
                if not existing_playlist:
                    ...
                    for position, song_index in enumerate(song_indices):
                        ...
                        """), {
                            ...
                            'added_at': datetime.utcnow() - timedelta(days=random.randint(1, 100))
                        })
                else:
