...
class SpotifyService:
    ...
    def _refresh_token(self):
        ...
        try:
            response = requests.post(
                "https://accounts.spotify.com/api/token", data=token_data, timeout=15
            )
            ...
            self.user.token_expiry = datetime.now(timezone.utc) + timedelta(
    ...
    def get_user_playlists(self) -> List[Dict[str, Any]]:
        ...
        fields = (
            "items(id,name,description,images,owner(id),collaborative,tracks(total)),next,total"
        )
    ...
    def get_playlist_tracks(self, playlist_id: str) -> List[Dict[str, Any]]:
        ...
        while url:
            ...
            valid_tracks = [
                item for item in data["items"] if item["track"] and item["track"].get("id")
            ]
    ...
    def sync_user_playlists(self) -> int:
        ...
        for spotify_playlist in spotify_playlists:
            ...
            if spotify_playlist["owner"]["id"] != self.user.spotify_id and not spotify_playlist.get(
                "collaborative", False
            ):
                continue
            ...
            playlist = Playlist.query.filter_by(
                owner_id=self.user.id, spotify_id=spotify_playlist["id"]
            ).first()
            ...
            if not playlist:
                ...
                playlist.image_url = (
                    spotify_playlist["images"][0]["url"] if spotify_playlist["images"] else None
                )
    ...
    def _sync_playlist_tracks(self, playlist: Playlist, spotify_playlist_id: str):
        ...
        try:
            ...
            for position, track_item in enumerate(spotify_tracks):
                ...
                new_playlist_songs.append({"song": song, "position": position})
            ...
            for ps_data in new_playlist_songs:
                ...
                if not existing_ps:
                    ...
                    playlist_song = PlaylistSong(
                        playlist_id=playlist.id, song_id=song.id, track_position=position
                    )
            ...
            current_app.logger.error(
                f"Error syncing playlist tracks for playlist {playlist.id}: {e}"
            )
            db.session.rollback()
            raise
    ...
    def remove_song_from_playlist(self, spotify_playlist_id: str, song_id: int) -> bool:
        ...
        try:
            ...
            # Remove from Spotify
            data = {"tracks": [{"uri": f"spotify:track:{song.spotify_id}"}]}

            self._make_request("DELETE", f"playlists/{spotify_playlist_id}/tracks", json=data)
