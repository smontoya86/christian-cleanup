...
class EnhancedConcernDetector:
    ...
    def __init__(self):
        ...
        self.concern_patterns = {
            ...
            "substance_abuse": {
                ...
                "severity": "medium",
                "patterns": [
                    # Specific substance abuse contexts (broad + contextual)
                    r"\b(drunk|drinking|alcohol|beer|wine|high|weed|marijuana|drugs|pills)\b",
                    r"\b(drunk|drinking|alcohol|beer|wine)\s+(heavily|everyday|tonight|to forget|to numb)\b",
        ...
        for concern_type, config in self.concern_patterns.items():
            ...
            if concern_matches:
                ...
                concern_detail = {
                    ...
                    "alternative_approach": "Choose content that promotes holiness, edifying language, and self-control consistent with Scripture.",
                    "biblical_perspective": config["biblical_perspective"],
                    "educational_value": f"This content shows {config['category'].lower()} concerns. {config['explanation']} Biblical guidance: {config['biblical_perspective']} Consider: Choose words that encourage and edify others, reflecting God's character."
                    if concern_type == "explicit_language"
                    else f"This content shows {config['category'].lower()} concerns. {config['explanation']} Biblical guidance: {config['biblical_perspective']} Consider: Focus on themes that align with biblical values and spiritual growth.",
    ...
    def _calculate_overall_concern_level(self, detected_concerns: List[Dict]) -> str:
        ...
        high_severity_count = sum(
            1 for concern in detected_concerns if concern.get("severity") == "high"
        )
        medium_severity_count = sum(
            1 for concern in detected_concerns if concern.get("severity") == "medium"
        )
    ...
    def _generate_educational_summary(
        self, detected_concerns: List[Dict], title: str, artist: str
    ) -> str:
    ...
    def analyze_content_concerns(self, title: str, artist: str, lyrics: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "discernment_guidance": [
                concern.get("biblical_perspective", "") for concern in result["concern_flags"]
            ],
    ...
    def _generate_educational_explanation(
        self, concern_type: str, concern_data: Dict, matches: List[str]
    ) -> str:
    ...
    def _generate_discernment_guidance(self, concerns: List[Dict]) -> List[str]:
        ...
        for category in categories:
            if "Language" in category:
                guidance.append(
                    "Consider whether the language used honors God and builds others up."
                )
            elif "Purity" in category:
                guidance.append(
                    "Evaluate whether this content promotes pure thoughts and God-honoring relationships."
                )
            elif "Substance" in category:
                guidance.append(
                    "Reflect on whether this content encourages healthy, God-honoring choices."
                )
            elif "Violence" in category:
                guidance.append(
                    "Consider whether this content promotes peace and forgiveness over conflict."
                )
            elif "Materialism" in category:
                guidance.append(
                    "Ask whether this content encourages contentment and trust in God's provision."
                )
            elif "Pride" in category:
                guidance.append(
                    "Evaluate whether this content promotes humility and recognition of God's grace."
                )
            elif "Occult" in category:
                guidance.append(
                    "Ensure this content doesn't promote spiritual practices contrary to Christianity."
                )
            elif "Despair" in category:
                guidance.append(
                    "Consider whether this content ultimately points toward hope in God or away from it."
                )
            elif "Rebellion" in category:
                guidance.append(
                    "Reflect on whether this content respects God-given authority and promotes biblical submission."
                )
            elif "False Teaching" in category:
                guidance.append(
                    "Verify that this content aligns with core Christian doctrines and biblical truth."
                )
    ...
    def _generate_recommendation(self, concern_level: str, concerns: List[Dict]) -> str:
        ...
        if concern_level == "Very Low":
            return (
                "This content is suitable for Christian listeners and can support spiritual growth."
            )
