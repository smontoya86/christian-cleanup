...
class EnhancedScriptureMapper:
    ...
    def __init__(self):
        ...
        self.theme_scripture_map = {
            ...
            'judgment': {
                ...
                'supporting_verses': [
                    ...
                    ('Matthew 25:31-32', 'When the Son of Man comes in his glory, and all the angels with him, he will sit on his glorious throne'),
            ...
            'hope': {
                ...
                'supporting_verses': [
                    ('Romans 8:24-25', 'For in this hope we were saved. But hope that is seen is no hope at all'),
            ...
            'provision': {
                ...
                'supporting_verses': [
                    ...
                    ('1 Kings 17:14', 'For this is what the Lord, the God of Israel, says: \'The jar of flour will not be used up and the jug of oil will not run dry\''),
            ...
            'covenant': {
                ...
                'supporting_verses': [
                    ('Luke 22:20', 'In the same way, after the supper he took the cup, saying, "This cup is the new covenant in my blood"'),
        ...
        logger.info(f"Enhanced Scripture Mapper initialized with {len(self.theme_scripture_map)} comprehensive theme mappings")
    ...
    def get_comprehensive_scripture_references(self, positive_themes: List[str], concern_themes: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if concern_themes:
            ...
            for concern in concern_themes:
                theme_name = concern.get('category', 'general_concern')
                biblical_response = self._get_biblical_response_to_concern(theme_name)
                if biblical_response:
                    concern_responses.append({
                        'concern': concern,
                        'biblical_response': biblical_response
                    })
            result['concern_responses'] = concern_responses
        
        # Generate summary
        result['summary'] = {
            'total_positive_themes': len(positive_themes),
            'total_concerning_themes': len(concern_themes),
            'total_scripture_references': len(result['positive_references']) + len(result['concern_responses']),
            'coverage_assessment': self._assess_theme_coverage(positive_themes)
        }
        
        # Add educational insights
        result['educational_insights'] = self._generate_educational_insights(positive_themes, concern_themes)
    ...
    def _assess_theme_coverage(self, themes: List[str]) -> str:
        ...
        theological_categories = {
            ...
            'soteriology': ['salvation', 'grace', 'forgiveness', 'faith', 'repentance', 'justification', 'sanctification'],
            'ecclesiology': ['church', 'worship', 'prayer', 'fellowship'],
            'eschatology': ['heaven', 'second_coming', 'judgment', 'eternal'],
            'christian_living': ['love', 'peace', 'joy', 'hope', 'patience', 'humility', 'strength']
        }
    ...
    def _generate_educational_insights(self, positive_themes: List[str], concern_themes: List[Dict]) -> List[str]:
        ...
        if 'god' in positive_themes and 'jesus' in positive_themes:
            insights.append("This song demonstrates a Trinitarian understanding of God, referencing both the Father and Son.")
        
        if 'salvation' in positive_themes and 'grace' in positive_themes:
            insights.append("The song reflects the biblical doctrine of salvation by grace, not by works.")
        
        if 'worship' in positive_themes and 'love' in positive_themes:
            insights.append("This song combines vertical worship (toward God) with horizontal love (toward others).")
        
        if 'cross' in positive_themes and 'resurrection' in positive_themes:
            insights.append("The song encompasses the full gospel message from crucifixion to resurrection.")
        
        if 'trials' in positive_themes and 'comfort' in positive_themes:
            insights.append("This song addresses the reality of suffering while pointing to God's comfort.")
        
        # Insights about theological depth
        if len(positive_themes) >= 5:
            insights.append("This song demonstrates significant theological depth with multiple Christian themes.")
        elif len(positive_themes) >= 3:
            insights.append("This song shows good theological content with several Christian themes.")
        
        # Insights about concerns
        if concern_themes:
            insights.append("Consider the concerning elements in light of biblical teaching for balanced understanding.")
