...
# Local enum definitions (queue system removed)
class JobType(Enum):
...
class ETACalculator:
    ...
    def record_completion(self, job_type: JobType, duration_per_item: float) -> None:
        ...
        if len(self.historical_data[job_type]) > self.max_history_size:
            self.historical_data[job_type] = self.historical_data[job_type][
    ...
    def calculate_eta(
        self, job_type: JobType, total_items: int, completed_items: int, elapsed_time: float
    ) -> float:
    ...
    def redis(self):
        ...
        if self._redis is None:
            try:
                ...
                redis_url = current_app.config.get("RQ_REDIS_URL") or current_app.config.get(
                    "REDIS_URL"
                )
                if not redis_url:
...
class ProgressTracker:
    ...
    def start_job_tracking(
        ...
        job_type: JobType,
        total_items: int,
        estimated_duration_per_item: float = None,
    ) -> JobProgress:
    ...
    def update_job_progress(
        ...
        update = ProgressUpdate(
            ...
            message=message
            or progress.current_message
            or f"Progress: {progress.completed_items}/{progress.total_items}",
        ...
        # Save to persistence
        self.persistence.save_progress(progress)
        # Heartbeat for watchdog/UI
        try:
            if self.persistence.redis is not None:
                self.persistence.redis.hset(
                    f"{self.persistence.key_prefix}{job_id}",
                    mapping={"heartbeat": int(__import__("time").time())},
                )
        except Exception:
            pass
