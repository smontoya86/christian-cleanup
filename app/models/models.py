...
class Playlist(db.Model):
    ...
    )  # Tracks when the playlist was last synced from Spotify changes
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    updated_at = db.Column(
        db.DateTime,
        default=lambda: datetime.now(timezone.utc),
    ...
    def songs(self):
        ...
        return [
            association.song
            for association in sorted(self.song_associations, key=lambda x: x.track_position)
        ]
...
class AnalysisResult(db.Model):
    ...
    )  # Positive Christian themes detected
    biblical_themes = db.Column(db.JSON, nullable=True)  # Biblical themes and relevant verses
    supporting_scripture = db.Column(
        db.JSON, nullable=True
    )  # Supporting scripture references and text

    # v3.1 enhanced framework fields
    verdict = db.Column(
        db.String(50), nullable=True
    )  # freely_listen|context_required|caution_limit|avoid_formation
    purity_score = db.Column(db.Float, nullable=True)
    formation_risk = db.Column(db.String(20), nullable=True)  # very_low|low|high|critical
    doctrinal_clarity = db.Column(db.String(20), nullable=True)  # sound|thin|confused|null
    confidence = db.Column(db.String(20), nullable=True)  # high|medium|low
    needs_review = db.Column(db.Boolean, default=False)
    narrative_voice = db.Column(db.String(20), nullable=True)  # artist|character|ambiguous
    lament_filter_applied = db.Column(db.Boolean, default=False)
    framework_data = db.Column(db.JSON, nullable=True)  # raw framework-derived details
    ...
    def positive_themes_identified_parsed(self):
        ...
        if self.positive_themes_identified:
            # Already parsed since it's a JSON column
            return (
                self.positive_themes_identified
                if isinstance(self.positive_themes_identified, list)
                else []
            )
        return []
...
class Whitelist(db.Model):
    ...
    )  # For song, playlist, or artist Spotify ID
    ...
    __table_args__ = (
        db.UniqueConstraint("user_id", "spotify_id", "item_type", name="_user_spotify_item_uc"),
    )
...
class Blacklist(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "user_id", "spotify_id", "item_type", name="_user_spotify_item_blacklist_uc"
        ),
    )
...
class LyricsCache(db.Model):
    ...
    def find_cached_lyrics(cls, artist, title):
        ...
        return cls.query.filter(
            db.func.lower(cls.artist) == artist_lower, db.func.lower(cls.title) == title_lower
        ).first()
    ...
    def cache_lyrics(cls, artist, title, lyrics, source):
        ...
        if existing:
            ...
            # Create new entry
            cache_entry = cls(artist=artist, title=title, lyrics=lyrics, source=source)
