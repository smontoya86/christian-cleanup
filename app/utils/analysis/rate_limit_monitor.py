...
class RateLimitMonitor:
    ...
    def check_rate_limit(self, identifier: str = "default") -> Dict[str, any]:
        ...
        with self._lock:
            ...
            minute_requests = sum(
                1 for ts in self._request_history[identifier] if ts > current_time - 60
            )
            ...
            if self._active_requests >= self.limits["concurrent_requests"]:
                limits_status.update(
                    {"allowed": False, "reason": "Too many concurrent requests", "retry_after": 5}
                )
            ...
            if daily_requests >= self.limits["requests_per_day"]:
                limits_status.update(
                    {"allowed": False, "reason": "Daily quota exceeded", "retry_after": 86400}
                )
                return limits_status
    ...
    def complete_request(self, identifier: str = "default") -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"Request completed for {identifier}. " f"Active: {self._active_requests}")
    ...
    def get_usage_stats(self, identifier: str = "default") -> Dict[str, any]:
        ...
        with self._lock:
            ...
            minute_requests = sum(1 for ts in self._request_history[identifier] if ts > now - 60)
            hour_requests = len(self._request_history[identifier])
            daily_requests = self._daily_counts[identifier]
            ...
            return {
                ...
                "percentages": {
                    ...
                    "concurrent": (self._active_requests / self.limits["concurrent_requests"])
                    * 100,
                },
            }
