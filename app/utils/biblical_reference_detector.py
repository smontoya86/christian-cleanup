...
class EnhancedBiblicalDetector:
    ...
    def _setup_scripture_patterns(self):
        ...
        book_pattern = "|".join(
            re.escape(book) for book in sorted(all_book_patterns, key=len, reverse=True)
        )
        ...
        self.compiled_scripture_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.scripture_patterns
        ]
    ...
    def _setup_enhanced_themes(self):
        ...
        for theme, data in self.enhanced_themes.items():
            self.compiled_theme_patterns[theme] = [
    ...
    def analyze_lyrics(self, lyrics: str) -> Dict[str, Any]:
        ...
        try:
            ...
            biblical_score = self._calculate_biblical_score(
                scripture_refs, biblical_names, themes, verse_matches, concepts
            )
        ...
        for pattern in self.compiled_scripture_patterns:
            ...
            for match in matches:
                ...
                for canon_name, abbreviations in self.bible_books.items():
                    if book == canon_name.lower() or book in [
                        abbr.lower() for abbr in abbreviations
                    ]:
        ...
        for theme_name, theme_data in self.enhanced_themes.items():
            ...
            for pattern in patterns:
                ...
                for match in pattern_matches:
                    ...
                    matches.append(
                        {"text": match.group(0), "context": context, "position": match.start()}
                    )
    ...
    def _detect_verse_content(self, lyrics: str) -> List[Dict[str, Any]]:
        ...
        return sorted(verse_matches, key=lambda x: x["combined_score"], reverse=True)[
            :5
        ]  # Top 5 matches
    ...
    def _calculate_biblical_score(
        self, scripture_refs: List, names: List, themes: List, verse_matches: List, concepts: List
    ) -> int:
    ...
    def _generate_supporting_scripture(
        self, themes: List[Dict], scripture_refs: List[Dict]
    ) -> Dict[str, Any]:
