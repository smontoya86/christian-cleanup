...
from werkzeug.exceptions import HTTPException, NotFound, BadRequest, Unauthorized, Forbidden, Conflict, TooManyRequests
...
def register_error_handlers(app: Flask) -> None:
    ...
    def handle_not_found(error: NotFound):
        ...
        if request and request.path:
            ...
            if len(path_parts) >= 2:
                resource_type = path_parts[-2].rstrip('s').title()  # Remove trailing 's' and capitalize
