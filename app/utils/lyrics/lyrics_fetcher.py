...
class LyricsFetcher:
    def __init__(self, genius_token=None, config: Optional[LyricsFetcherConfig] = None):
        ...
        self.genius_token = genius_token or os.getenv('LYRICSGENIUS_API_KEY') or os.getenv('GENIUS_ACCESS_TOKEN')
        ...
        self._cache_batch = []
        self._batch_size = getattr(config, 'cache_batch_size', 50)  # Default batch size
        self._batch_timeout = getattr(config, 'cache_batch_timeout', 30)  # Seconds to wait before forcing commit
    ...
    def _add_to_cache_batch(self, cache_key: str, lyrics: Optional[str]) -> None:
        ...
        try:
            ...
            if self.config.log_cache_operations:
                logger.debug(f"Added to cache batch: '{artist}' - '{title}' (batch size: {len(self._cache_batch)})")
            
            # Check if we should flush the batch
            current_time = time.time()
            batch_age = current_time - self._last_batch_time
            
            if (len(self._cache_batch) >= self._batch_size or 
                batch_age >= self._batch_timeout):
    ...
    def _flush_cache_batch(self) -> None:
        ...
        try:
            ...
            for operation in self._cache_batch:
                try:
                    ...
                    if not lyrics:
                        ...
                        if self.config.log_cache_operations:
                            logger.debug(f"Batched cache for '{artist}' - '{title}' (source: {source}, {len(lyrics)} chars)")
            ...
            if self.config.log_cache_operations:
                logger.info(f"Flushed cache batch: {batch_size} operations ({successful_ops} positive, {negative_ops} negative) in single commit")
            
            # Record batch metrics
            self.metrics.record_event('cache_batch_flushed', 
    ...
    def _store_in_cache(self, cache_key: str, lyrics: Optional[str], ttl: int = None) -> None:
        ...
        # Check if batching is enabled (batch_size > 1)
    ...
    def fetch_lyrics(self, title: str, artist: str, force_refresh: bool = False) -> Optional[str]:
        ...
        if not force_refresh and self.config.use_cache:
            ...
            elif cached_lyrics is None and self._is_negative_cache_hit(cache_key):
                # This is a negative cache hit (we know this song has no available lyrics)
                self.metrics.record_fetch_time('negative_cache_hit', time.time() - start_time)
                return None
        ...
        for provider in self.providers:
            ...
            try:
                if self.config.log_api_calls:
                    logger.debug(f"Attempting to fetch lyrics from {provider_name} for '{title}' by {artist}")
                
                provider_start = time.time()
                lyrics = provider.fetch_lyrics(title, artist)
                ...
                if lyrics:
                    ...
                    if self.config.log_api_calls:
                        logger.info(f"Successfully fetched lyrics from {provider_name} for '{title}' by {artist} ({len(lyrics)} characters)")
                    break
                else:
                    self.metrics.record_provider_failure(provider_name, "no_lyrics_found")
                    if self.config.log_api_calls:
                        logger.debug(f"No lyrics found by {provider_name} for '{title}' by {artist}")
                ...
                logger.warning(f"Error with provider {provider_name} for '{title}' by {artist}: {error_msg}")
                continue
        ...
        if lyrics:
            ...
            if errors and self.config.log_api_calls:
                logger.warning(f"All providers failed for '{title}' by {artist}. Errors: {'; '.join(errors)}")
        ...
        try:
            ...
            if self.config.log_provider_metrics:
                ...
                for provider_name, stats in self.provider_stats.items():
                    ...
                    logger.info(f"  {provider_name}: {successes}/{attempts} ({success_rate:.1f}% success rate)")
        except Exception as e:
