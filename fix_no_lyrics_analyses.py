...
from app.models.models import AnalysisResult, Song, User, Playlist, PlaylistSong
...
def clear_no_lyrics_analyses():
    ...
    with app.app_context():
        ...
        songs_without_lyrics = db.session.query(Song.id).filter(
            db.or_(
                Song.lyrics == None,
                Song.lyrics == '',
        ...
        no_lyrics_count = db.session.query(AnalysisResult).filter(
            AnalysisResult.song_id.in_(db.session.query(songs_without_lyrics.c.id))
        ).count()
        ...
        deleted = db.session.query(AnalysisResult).filter(
            AnalysisResult.song_id.in_(db.session.query(songs_without_lyrics.c.id))
...
def test_lyrics_fetching():
    ...
    with app.app_context():
        ...
        print(f"✅ Genius API key configured")
...
def trigger_reanalysis(user_id=None):
    ...
    with app.app_context():
        ...
        try:
            ...
            print(f"🌐 Monitor progress at: http://localhost:5001/dashboard")
